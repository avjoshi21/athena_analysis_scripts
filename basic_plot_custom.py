  ...
  **kwargs : dict
  ...
  for ind,variableName in enumerate(variables):
    ...
    for i in range(slice_grid.shape[0]):
        ...
        if(log_flag):
          im=ax.pcolormesh(slice_grid[i,:,0]/length_scale,slice_grid[i,:,1]/length_scale,np.log10(abs(data_toplot)),**kwargs)
        else:
          im=ax.pcolormesh(slice_grid[i,:,0]/length_scale,slice_grid[i,:,1]/length_scale,data_toplot,**kwargs)
...
def process_file(ath_file,plot_type,slice_kwargs,plot_kwargs):
  ...
  """
  variables=plot_kwargs['variables']
  ...
  # #   plot_panel_slice(ath_data, ath_file=ath_file, **plot_kwargs)
...
def load_and_plot(files_and_kwargs,plot_type,num_cores_multiplier,parallel,variables,slice_dim,loc,slice_domain,length_scale,length_scale_label,output_dir):
  ...
  slice_coord_dims = list(set(range(3))-{slice_dim})
  slice_coords_list = ["x{i+1}" for i in slice_coord_dims]
